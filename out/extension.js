...
function activate(context) {
    const disposable = vscode.commands.registerCommand("anno117.translateXml", async () => {
        const config = vscode.workspace.getConfiguration("anno117translator");
        const BATCH_DELAY = config.get("batchDelay") ?? 250;
        let DEEPL_API_KEY = config.get("deepLApiKey");
        const overwriteSetting = config.get("overwriteExisting") ?? false;
        ...
        if (!DEEPL_API_KEY) {
            ...
            if (!DEEPL_API_KEY) {
                vscode.window.showErrorMessage("DeepL API key is required to run the translation.");
                return;
            }
        }
        ...
        if (!document.fileName.endsWith("texts_english.xml")) {
            vscode.window.showErrorMessage("Please open texts_english.xml before running this command.");
            return;
        }
        ...
        await vscode.window.withProgress({
            ...
            for (const [langName, langCode] of Object.entries(LANGUAGES)) {
                ...
                translatedXml = await translateAllTextNodes(translatedXml, langCode, DEEPL_API_KEY, BATCH_DELAY, progress);
                // PASS 2: translate raw <ModOp> text
                translatedXml = await translateRawModOpText(translatedXml, langCode, DEEPL_API_KEY, BATCH_DELAY, progress);
...
// PASS 1: translate <Text> nodes
//
async function translateAllTextNodes(xml, targetLang, apiKey, batchDelay, progress) {
    ...
    while ((match = regex.exec(xml)) !== null) {
        ...
        for (let i = 0; i < textNodes.length; i++) {
            ...
            if (translated)
                newInner = newInner.replace(text, translated);
            if (progress) {
...
// PASS 2: translate raw <ModOp> text
//
async function translateRawModOpText(xml, targetLang, apiKey, batchDelay, progress) {
...
//
async function translateBatch(texts, targetLang, apiKey, batchDelay) {
    const result = new Map();
    const toTranslate = texts.filter(t => !translationCache.has(`${targetLang}:${t}`));
    ...
    const response = await axios_1.default.post(DEEPL_ENDPOINT, params.toString(), {
        headers: {
